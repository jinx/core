...
module Jinx
  ...
  module Dependency
    ...
    #
    # The flags qualify the dependency for persistence adapters, e.g.:
        ...
        lambda do |ref|
          ...
          if prev and prev != ref then
            if ref.nil? then
              ...
              if self.class.name =~ /Registration/
                puts "dep bad reg pnt reset\n#{caller.map { |l| l + '<br/>' }}"
                raise StandardError.new("Reset the #{self} owner #{attribute} from #{prev} to #{ref}")
              end
